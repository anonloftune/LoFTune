...
class FactScorer(object):
    ...
    def __init__(self,
        ...
        if "llama" in model_name:
            ...
            self.lm = CLM("meta-llama/Meta-Llama-3-8B-Instruct",
                            cache_file=os.path.join(cache_dir, "Meta-Llama-3-8B-Instruct.pkl"))
    ...
    def register_knowledge_source(self, name="enwiki-20230401", db_path=None, data_path=None, searcher=None):
        if name == "elastic":
            self.retrieval[name] = searcher
    ...
    def get_score(self,
        ...
        if knowledge_source == "elastic":
            self.register_knowledge_source(knowledge_source, searcher=searcher)

        elif knowledge_source not in self.retrieval:
        ...
        #     self.print_cost_estimates(total_words, task="factscore evaluation", model="gpt-4o-2024-08-06")
        ...
        decisions = []
        # discarted_decisions = []
        for topic, generation, facts in tqdm(zip(topics, generations, atomic_facts)):
            if facts is None:
                decisions.append(None)
            else:
                ## topic can be a list
                if type(topic)==list:
                    ## decision_map contains a dict of claim:is_supported, for each iteration, will be done for those in is_supported==False,
                    decision_dict = {fact:None for fact in facts}
                    remaining_facts = [fact for fact in facts]
                    for topic_elem in topic:
                        for d in self._get_score(topic_elem, generation, remaining_facts, knowledge_source, can_apply_npm = False):
                            if d["is_supported"] is not None:
                                decision_dict[d["atom"]]=d["is_supported"]
                            if d["is_supported"]:
                                remaining_facts.remove(d["atom"])
                        if len(remaining_facts)==0:
                            break
                    decision = [{"atom":key,"is_supported":value,"topic":topic} for key,value in decision_dict.items()]
                    # discarted = [{"atom":key,"is_supported":value,"topic":topic} for key,value in decision_dict.items() if value is None]
                else:
                    decision = [{**d,"topic":topic} for d in self._get_score(topic, generation, facts, knowledge_source)]
                    # discarted = [{**d,"topic":topic} for d in self._get_score(topic, generation, facts, knowledge_source) if d["is_supported"] is None]
                
                score = np.mean([d["is_supported"]==True for d in decision])
                ...
                # discarted_decisions.append(discarted)
                scores.append(score)
                # if len(scores) % 10 == 0:
                #     self.save_cache()
        ...
        out = {"score": np.nanmean(scores),
               "respond_ratio": respond_ratio,
               "decisions": decisions,
               # "discarted_decisions": discarted_decisions,
               "num_facts_per_response": np.mean([len(d) for d in decisions]),
               "avg_supported_per_response": np.mean([len([d_item for d_item in d if d_item["is_supported"]==True])
                                                         for d in decisions]),
               "avg_refute_per_response": np.mean([len([d_item for d_item in d if d_item["is_supported"]==False])
                                                         for d in decisions]),
               "avg_nei_per_response": np.mean([len([d_item for d_item in d if d_item["is_supported"]==None])
                                                         for d in decisions]),
              }
    ...
    def _get_score(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None, can_apply_npm=True):
        decisions = []
        total_words = 0
...
Input: given the claim "{}" your task is to figure out if the claim is either supported, refuted or unrelated based exclusively on the evidence. Please structure the output as a JSON object, ensuring that all fields are included and meticulously populated according to the following structure (fiill with \"null\" if the field is missing):

    class Response(str, Enum):
        support = 'support' # the claim is supported by the evidence
        refute = 'refute' # the evidence directly contradicts the claim, if the claim is just not mentioned in the evidence, or if it is unrelated to the evidence, it does not mean it is refuted. For that cases, the answer will be "not_enough_information".
        not_enough_information = 'not_enough_information' # the evidence does not contain enough information or if it is not related to the claim
    
    class Output(BaseModel):
        response: Response
        rationale: str

Provide only the JSON object. Exclude comments and extra text. Be as concise as possible.
...
Output:""".format(definition.strip(), atom.strip())
                ...
                output_json = json_repair.repair_json(output[0], return_objects=True)
                if output_json == '':
                    print("Error parsing response from json output")
                    is_supported = None
                else:
                    match output_json["response"]:
                        case "support":
                            is_supported = True
                        case "refute":
                            is_supported = False
                ...
                #     assert logits.shape[0] in [128256]
                #     true_score = logits[3082]
                #     false_score = logits[3641]
            ...
            if is_supported and "npm" in self.model_name and can_apply_npm:
                print("Applying npm")
    ...
    def _get_score_batch(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None, can_apply_npm=True):
        decisions = []
        total_words = 0
        prompts = []
        for atom in atomic_facts:
...
Output:""".format(definition.strip(), atom.strip())
                ...
                ##########################################################################################
                # print(prompt)
        output = self.lm.generate_batch(prompts)
        # print(output[0])
        # print("-"*20)
        for i,atom in enumerate(atomic_facts):
            atom = atom.strip()
            if self.lm:
                output_json = json_repair.repair_json(output[i][0], return_objects=True)
                if output_json == '':
                    print("Error parsing response from json output")
                    is_supported = None
                else:
