...
def sample_model_questions(question_dataset_path=None, output_path=None, prompt = None, is_chat=False):
    questions_ds = load_from_disk(question_dataset_path)
    if is_chat:
        all_prompts = [[{"role": "system","content": "You are a helpful assistant. Answer in a short paragraph."},{'role': 'user', 'content': question}] for question in questions_ds["input"]]
        outputs = llm.chat(messages=all_prompts, sampling_params=sampling_params)
    else:
        question_ds_prompted = questions_ds.map(lambda ex: {"prompt":prompt.replace("{question}",ex["input"])})
